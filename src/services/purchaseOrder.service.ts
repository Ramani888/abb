...
export const updatePurchaseOrderData = async (data: IPurchaseOrder) => {
    try {
        ...
        const presentField = exclusiveFields.find(field => data[field as keyof IPurchaseOrder]);
        ...
        exclusiveFields.forEach(field => {
            if (field === presentField && data[field as keyof IPurchaseOrder]) {
                setFields[field] = data[field as keyof IPurchaseOrder];
            } else {
                unsetFields[field] = "";
            }
        });
        ...
        Object.keys(data).forEach(key => {
            if (!exclusiveFields.includes(key) && key !== "_id") {
                setFields[key] = data[key as keyof IPurchaseOrder];
            }
        });
        ...
        const result = await PurchaseOrder.findByIdAndUpdate(documentId, updateObj, {
            new: true,
            runValidators: true
        });
        return result;
